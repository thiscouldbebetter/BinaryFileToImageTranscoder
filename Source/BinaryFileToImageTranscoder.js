...
(function (ThisCouldBeBetter) {
    ...
    (function (BinaryFileToImageTranscoder_1) {
        class BinaryFileToImageTranscoder {
            ...
            bytesToCanvas(bytesAsStringHexadecimal) {
                ...
                var imageWidthInPixels = Math.ceil(Math.sqrt(numberOfPixels));
                var imageHeightInPixels = Math.ceil(numberOfPixels / imageWidthInPixels);
                var imageSizeInPixels = new BinaryFileToImageTranscoder_1.Coords(imageWidthInPixels, imageHeightInPixels + 1);
                var d = document;
                ...
                var pixelColorAsString = "rgb(" + numberOfBytesAsRGB.join(",") + ")";
                ...
                for (var y = 1; y < imageSizeInPixels.y; y++) {
                    for (var x = 0; x < imageSizeInPixels.x; x++) {
                        ...
                        var pixelColorAsString = "rgb(" + pixelColorRGB.join(",") + ")";
                        graphics.fillStyle = pixelColorAsString;
            ...
            saveImgElementToFileWithName(imgElement, fileName) {
                ...
                new BinaryFileToImageTranscoder_1.FileHelper().saveCanvasToFileWithName(canvas, fileName + ".png");
            }
            imgElementToBytes(imgElement) {
                var imageSizeInPixels = new BinaryFileToImageTranscoder_1.Coords(imgElement.width, imgElement.height);
                var canvas = document.createElement("canvas");
                canvas.width = imageSizeInPixels.x;
                canvas.height = imageSizeInPixels.y;
                var graphics = canvas.getContext("2d");
                ...
                var bytesPerPixel = 3;
                var bytes = [];
                var pixelColorsRGBA = graphics.getImageData(0, 0, imageSizeInPixels.x, imageSizeInPixels.y).data;
                // Decode the file's length in bytes from the upper-left pixel.
                var numberOfBytesAsRGB = pixelColorsRGBA.slice(0, 3);
                var numberOfBytes = (numberOfBytesAsRGB[0] << 16)
                    + (numberOfBytesAsRGB[1] << 8)
                    + (numberOfBytesAsRGB[2]);
                var byteIndex = 0;
                for (var y = 1; y < imageSizeInPixels.y; y++) {
                    for (var x = 0; x < imageSizeInPixels.x; x++) {
                        var pixelIndex = y * imageSizeInPixels.x
                            + x;
                        var pixelColorRGBA = pixelColorsRGBA.slice(pixelIndex * 4, pixelIndex * 4 + 4);
                        for (var b = 0; b < bytesPerPixel; b++) {
            ...
            saveBytesFromHexadecimalToFileWithName(bytesAsStringHexadecimal, fileName) {
                var bytes = this.bytesFromStringHexadecimal(bytesAsStringHexadecimal);
                new BinaryFileToImageTranscoder_1.FileHelper().saveBytesToFileWithName(bytes, fileName);
            }
        }
        BinaryFileToImageTranscoder_1.BinaryFileToImageTranscoder = BinaryFileToImageTranscoder;
